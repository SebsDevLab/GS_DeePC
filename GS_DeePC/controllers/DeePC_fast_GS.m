...
%       Predictive Control for Nonlinear Systems with Linearized 
%       Operating Regions",
...
% control behavior.
% Furthermore compared to DeePC_fast.m also the Hankel-matrix H_u and H_y
% are precompiled via a placeholer and assigned later with the correct
% Hankelmatrix given the current operating point.
% Further Notes:
...
classdef DeePC_fast_GS < handle
    properties
        ...
        H_u_par;   % placeholder for Hankel(u)
        H_y_par;   % placeholder for Hankel(y)
    ...
    methods
        function obj = DeePC_fast_GS(T_d, T_ini, T_fut, R, Q, lambda_ini, lambda_g, ~, ~, sys)
        ...
        function initializeOptimization(obj)
            ...
            % --- PARAMETRIC Hankel matrix placeholders ---
            obj.H_u_par = sdpvar(obj.nu * obj.T, ncols_g , 'full');   % dynamic width
            obj.H_y_par = sdpvar(obj.ny * obj.T, ncols_g , 'full');
            U_p = obj.H_u_par(1 : obj.nu * obj.T_ini, :);
            U_f = obj.H_u_par(obj.nu * obj.T_ini + 1 : end, :);
            
            Y_p = obj.H_y_par(1 : obj.ny * obj.T_ini, :);
            Y_f = obj.H_y_par(obj.ny * obj.T_ini + 1 : end, :);
            ...
            obj.paramSolver = optimizer(cons, cost, obj.options, ...
                    {obj.u_past_par, obj.y_past_par, obj.y_ref_par, obj.H_u_par, obj.H_y_par}, ...
        ...
        function [u_fut, y_value, g_value, sigma_value, yalmip_error_flag] = step(obj, u_past, y_past, y_ref, H_u, H_y)
            ...
            [sol, diagnostics] = obj.paramSolver({u_past, y_past, y_ref, H_u, H_y});
