...
% control nonlinear systems with dwell time only acting backwards.
%
% Sources:
...
for k = 1:ksim-T_fut
    block_time = block_time - 1;    % reducing block_time every iteration
    ...
    if (r_idx < num_ranges &&  y_now >= lower && y_now < upper) || ...
       (r_idx == num_ranges && y_now >= lower && y_now <= upper)
        ...
        if r_idx_new == block_idx && block_time > 0
            % same region and no block_time active
            r_idx = r_idx;
        else
            % change of region and activate dwell_time
            block_idx = r_idx;
